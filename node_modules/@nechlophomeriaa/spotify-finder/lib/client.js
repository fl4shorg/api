...
class Client {
  ...
  constructor(opts) {
    ...
    this.authURL =
      opts && opts.auth ? opts.auth : "https://accounts.spotify.com/api/token";
  ...
  search(params, callback) {
    ...
    if (callback) {
      return Promise.resolve(
        this.fetch(this.endpoints.search, params)
      ).asCallback(callback);
    }
  ...
  getAlbum(id, opts, callback) {
    if (id.includes("playlist")) {
      let url = `${this.endpoints.playlists}/${
        id.split("playlist/")[1].split("?")[0]
      }`;
      url = opts && opts.tracks ? `${url}/tracks` : url;
      if (callback) {
        return Promise.resolve(this.fetch(url)).asCallback(callback);
      }
      return Promise.resolve(this.fetch(url));
    }
    if (id.includes("album")) {
      var ids = id.split("album/")[1].split("?")[0];
    } else {
      var ids = id;
    }
    let url = `${this.endpoints.albums}/${ids}`;
    url = opts && opts.tracks ? `${url}/tracks` : url;
  ...
  getAlbums(array_ids, callback) {
    ...
    if (callback) {
      return Promise.resolve(
        this.fetch(this.endpoints.albums, { ids })
      ).asCallback(callback);
    }
  ...
  getArtist(id, opts, callback) {
    ...
    /* istanbul ignore else  */
    if (callback) {
      return Promise.resolve(this.fetch(url, params)).asCallback(callback);
    }
  ...
  getArtists(array_ids, callback) {
    ...
    if (callback) {
      return Promise.resolve(
        this.fetch(this.endpoints.artists, { ids })
      ).asCallback(callback);
    }
  ...
  getTracks(array_ids, callback) {
    ...
    if (callback) {
      return Promise.resolve(
        this.fetch(this.endpoints.tracks, { ids })
      ).asCallback(callback);
    }
  ...
  getCategory(id, opts, callback) {
    ...
    /* istanbul ignore else  */
    if (callback) {
      return Promise.resolve(this.fetch(url, opts)).asCallback(callback);
    }
