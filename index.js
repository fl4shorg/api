...
"title": `Pterodactyl Server By `,
"description": null,
"currencyCode": "IDR",
"priceAmount1000": "9999999999",
"retailerId": `Powered By reyzoffc`,
"productImageCount": 1
},
...
const qpayment = {
  key: {
    ...
    fromMe: false,
    id: '0@s.whatsapp.net', // Isso deve ser o ID correto da mensagem
    participant: '0@s.whatsapp.net'
  },
  message: {
    requestPaymentMessage: {
      currencyCodeIso4217: "US",
      amount1000: 99999, // Ajuste o valor conforme necessário
      requestFrom: '0@s.whatsapp.net',
      noteMessage: {
        extendedTextMessage: {
          text: "PREFIXO" // Substitua pelo texto desejado
        }
      },
      expiryTimestamp: 1718487612000, // Ajuste para um timestamp válido
      amount: {
        value: 9192.92, // Valor ajustado para um formato realista
        offset: 1000,
        currencyCode: "INR"
      }
    }
...
const carrinho = {"key": {   "fromMe": false,"participant":"0@s.whatsapp.net",   "remoteJid": 'status@broadcast'  }, "message": {orderMessage: {itemCount: -727379969,status: true, surface: 200, message: `NEEXT`, orderTitle: 'kkkkk', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":false},sendEphemeral: false}

const convite = {"key": {   "fromMe": false,"participant":"0@s.whatsapp.net",   "remoteJid": 'status@broadcast'  }, message: {"groupInviteMessage": { "groupName": `neext`, "groupJid": '120363027411739028@g.us', "inviteCode": 'Gqpefh8MYacEhzLYkDPEp8', "inviteExpiration": '259200', "caption": 'neext'}}}
...
const floc0 = {
    ...
    message: {
        documentMessage: {
            name: `documento: menu completo`,
            thumbnailUrl: ftmenu.logo,
            caption: 'teste'  // Aqui você adiciona o texto desejado
        }
    }
};
...
const qchanel = {
  ...
  message: {
    newsletterAdminInviteMessage: {
      ...
      newsletterName: 'Hore',
      jpegThumbnail: ftmenu.logo, // Se você não tem um thumbnail, defina como null ou remova
      caption: ' prefixo💦 ',
      inviteExpiration: Date.now() + 1814400000 // 21 dias em milissegundos
    }
  }
};
		...
		const Shun = {
            ...
            message: {
                "contactMessage": {
                    ...
                    'jpegThumbnail': ftmenu.logo,
                    thumbnail: ftmenu.logo,
                    sendEphemeral: true
                }   
            }
        }
...
//selo skid
var seloblk = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ... {}}, message: {"contactMessage": {"displayName": `neext💗`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;neext💗,;;;\nFN: neext 💗,\nitem1.TEL;waid=${botNumber.split('@')[0]}:${botNumber.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
...
await akame.sendMessage(from, {text: linguagem.menu(prefix), contextInfo: {externalAdReply: {title: `〘🏮〙${NomeDoBot}〘🏮〙\n⿻ꦿꦂ𝐔𝐬𝐮𝐚𝐫𝐢𝐨: ${pushname}`, body: `Ⓘ Esse é o melhor bot do momento s꯭꯭꯭a꯭꯭i꯭b꯭꯭a꯭ ꯭꯭m꯭꯭꯭a꯭꯭꯭i꯭꯭꯭꯭s`, thumbnail: await getBuffer(`${ftmenu.logo}`), mediaType: 1, showAdAttribution: true, renderLargerThumbnail: true, sourceUrl: group}}}, {quoted: carrinho})
break 
...
{title: `「 🏮 」𝐌𝐄𝐍𝐔 𝐁𝐑𝐈𝐍𝐂𝐀𝐃𝐄𝐈𝐑𝐀𝐒「 🏮️ 」`, description: ` ${NomeDoBot} `, id: `${prefix}brincadeiras`},
...
akame.sendInteractiveIMG('./database/data/media/images/menulist-image.jpg', from, `❪⛩️ꦿ𝙼𝙴𝙽𝚄 𝙻𝙸𝚂𝚃ꯧ⸼❫\n్మాై𝚄𝚂𝚄𝙰𝚁𝙸𝙾: ${pushname}\n`, "© Neext ltda", {participant: sender, quotedMessage: Shun}, {buttons: [{name: "single_select", buttonParamsJson: JSON.stringify({title: "「 📄 」𝐌𝐄𝐍𝐔 𝐋𝐈𝐒𝐓 「 🏮 」", sections: [{title: `ぁ NEZUKO da neext ぁ`, highlight_label: "", rows: Lrows}]})}]});
break
...
case 'correio': {
    ...
    console.log('Número:', txt1);
    console.log('Mensagem:', txt2);
...
case 'playteste':
    case 'p':
        ...
        try {
            // Pesquisar o vídeo usando ytsr
            const searchResults = await ytsr(q, { limit: 1 });
            if (!searchResults.items || searchResults.items.length === 0) {
                return reply("Não foi possível encontrar o vídeo com este nome.");
            }
            ...
            // Obter informações do vídeo usando ytdl
            const info = await ytdl.getInfo(videoUrl);
            const thumbnailUrl = info.videoDetails.thumbnails[0].url;

            // Baixar e salvar a imagem da miniatura
            const imageBuffer = await getBuffer(thumbnailUrl);
            fs.writeFileSync('./database/data/media/images/play-thumb.jpg', imageBuffer);

            // Preparar e enviar a mensagem interativa
            akame.sendInteractiveIMG('./database/data/media/images/play-thumb.jpg', from, mess.playResult(info), "© nezuko neext", 
                {participant: sender, quotedMessage: info.message}, 
                {buttons: [
                    {name: "quick_reply", buttonParamsJson: JSON.stringify({display_text: "「 🏮 」𝐃𝐎𝐂𝐔𝐌𝐄𝐍𝐓𝐎「 🏮 」", id: `${prefix}ytdoc ${videoUrl}`})},
                    {name: "quick_reply", buttonParamsJson: JSON.stringify({display_text: "「 🏮 」𝐀𝐔𝐃𝐈𝐎「 🏮 」", id: `${prefix}ytmp3 ${videoUrl}`})},
                    {name: "quick_reply", buttonParamsJson: JSON.stringify({display_text: "「 🏮 」𝐕𝐈𝐃𝐄𝐎「 🏮 」", id: `${prefix}ytmp4 ${videoUrl}`})}
                ]}
            );
        } catch (error) {
            console.error(error);
...
var sti = new Buffer.from(mantap, 'base64');
await akame.sendMessage(from, {sticker: sti}, {quoted: Shun2}).catch(async(error) => {
reply(mess.error()); 
})
break
...
case 'rename':
    ...
    // Verifica se a mensagem contém o formato correto
    var names = args.join(" ").split('/');
    if (names.length < 2) {
        return reply('Por favor, forneça o novo nome para o pack e o autor no formato: `nome_do_pack/nome_do_autor`. Exemplo: `Figurinhas/João`.');
    }

    var packName = names[0].trim();
    var authorName = names[1].trim();

    try {
        // Obtendo o sticker mencionado
        var sticker = info.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage;

        // Convertendo o sticker para buffer
        var buff = await getFileBuffer(sticker, 'sticker');

        // Enviando o sticker renomeado
        let encmedia = await sendImageAsSticker2(akame, from, buff, Shun2, { packname: packName, author: authorName });
        await DLT_FL(encmedia);

    } catch (error) {
        console.log(error);
        reply('Ocorreu um erro ao renomear a figurinha.');
    }
    break;
...
var packnameStk = `↧ ❪🤖ฺ࣭࣪͘ꕸ▸ 𝐂𝐫𝐢𝐚𝐝𝐚 𝐩𝐨𝐫:\n• ↳ ${NomeDoBot}\n—\n↧ ❪🕵🏻‍♂️ฺ࣭࣪͘ꕸ▸ 𝐏𝐫𝐨𝐩𝐫𝐢𝐞𝐭𝐚𝐫𝐢𝐨:\n• ↳ ${ownerName}`
var authorSticker = `↧ ❪👤ฺ࣭࣪͘ꕸ▸ 𝐒𝐨𝐥𝐢𝐜𝐢𝐭𝐚𝐝𝐨 𝐩𝐨𝐫:\n• ↳ ${pushname}\n—\n↧ ❪🏮ฺ࣭࣪͘ꕸ▸ siga nosso Instagram @neet.tk:\n• ↳ `
reply(mess.wait())
...
if(boij2) {
var pack = `↧ ❪⭐ฺ࣭࣪͘ꕸ▸ 𝐂𝐫𝐢𝐚𝐝𝐚 𝐩𝐨𝐫:\n• ↳ ${NomeDoBot}\n—\n↧ 💻ฺ࣭࣪͘ꕸ▸ 𝐏𝐫𝐨𝐩𝐫𝐢𝐞𝐭𝐚𝐫𝐢𝐨:\n• ↳ ${ownerName}`
var author2 = `↧ ❪🏮ฺ࣭࣪͘ꕸ▸ 𝐒𝐨𝐥𝐢𝐜𝐢𝐭𝐚𝐝𝐨 𝐩𝐨𝐫:\n• ↳ ${pushname}\n—\n↧ ❪🏮ฺ࣭࣪͘ꕸ▸ siga nosso I instagram: @neet.tk :`
owgi = await getFileBuffer(boij2, 'image')
let encmediaa = await sendImageAsSticker2(akame, from, owgi, selo, { packname:pack, author:author2})
await DLT_FL(encmediaa)
} else if(boij && boij.seconds < 11){
var pack = `↧ ❪⭐ฺ࣭࣪͘ꕸ▸ 𝐂𝐫𝐢𝐚𝐝𝐚 𝐩𝐨𝐫:\n• ↳ ${NomeDoBot}\n—\n↧ 🏮️ฺ࣭࣪͘ꕸ▸ 𝐏𝐫𝐨𝐩𝐫𝐢𝐞𝐭𝐚𝐫𝐢𝐨:\n• ↳ ${ownerName}`
var author2 = `↧ ❪👤ฺ࣭࣪͘ꕸ▸ 𝐒𝐨𝐥𝐢𝐜𝐢𝐭𝐚𝐝𝐨 𝐩𝐨𝐫:\n• ↳ ${pushname}\n—\n↧ ❪🎨ฺ࣭࣪͘ꕸ▸ siga nosso Instagram:\n• ↳ @neet.tk`
owgi = await getFileBuffer(boij, 'video')
let encmedia = await sendVideoAsSticker2(akame, from, owgi, selo, { packname:pack, author:author2})
await DLT_FL(encmedia)
} else {
...
if (budy2 === "prefixo" && !isBotoff) {
    await reagir(from, "💦");
    await delay(1000);
    uptime = process.uptime();
    btncmd = [
        { type: `prefixo`, text: `「🧠 」𝐌𝐄𝐍𝐔 𝐌𝐀𝐋𝐢𝐆𝐍𝐎「🧠 」`, command: `${prefix}menu` },
    ];

    await sendButton(from, {
        text: `💦𝐏𝐑𝐄𝐅𝐈𝐗𝐎 𝐃𝐎 𝐁𝐎𝐓: *${prefix}* •°•°`,
        footer: ``
    }, akame, sender, btncmd, qchanel);
}
...
{type: `cmd`, text: `「  」𝐌𝐄𝐍𝐔 𝐋𝐈𝐒𝐓「  」`, command: `${prefix}menu`},
    ...
    await sendButton(from, {text: `🏮𝙳𝙸𝙶𝙸𝚃𝙴: *${prefix}𝙼𝙴𝙽𝚄*
╰──────────────✧ [🛡️] •°•°`, footer: ``}, akame, sender, btncmd, carrinho)}}}}
